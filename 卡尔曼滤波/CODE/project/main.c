...
KALMAN_T kalman;
void test(){
	float res = Kalman_Filter((Motor3.angle * 1.0),&kalman);
	printf("%d\r\n",(int)(res / 100));// / (4090/255));
	//printf("%d\n",Motor3.angle / 100); //(4090/255));
		/*while (USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET);
			USART_ClearFlag(USART1, USART_FLAG_TC);
			USART_SendData(USART1,(uint8_t)(res/10 / (4090/255)));*/
}
int main(void)
{
	//驱动板初始化
	...
	Kalman_Init(&kalman,0.001,0.942);
	//系统自检
	...
	//-----------计算电流task------------------*/
	TimeTask_add(create_SysTickStamp(0, 5, 0), ReadCurrent,Real_Time_Mode);	
	//--------------检查心跳task------------------
	...
	TimeTask_add(create_SysTickStamp(0, 10, 0), test,Real_Time_Mode);
