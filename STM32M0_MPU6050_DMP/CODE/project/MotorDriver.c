...
void MotorRingInit(MOTOR_PARM* motor,MOTOR_ID motor_id){
	
		memset(motor, 0, sizeof(MOTOR_PARM));
...
void MotorInt(void)	// 初始化电机参数 
{
	...
	MotorRingInit(&Motor1,ID_MOTOR_L);
	MotorRingInit(&Motor2,ID_MOTOR_R);
	Motor_H.MotorHeadInit(&Motor_H, ID_MOTOR_H, TIM14, GPIOB, GPIO_Pin_2);
	Motor_WL.MotorWingInit(&Motor_WL, ID_MOTOR_LC, TIM16, GPIOB, GPIO_Pin_10);
	Motor_WR.MotorWingInit(&Motor_WR, ID_MOTOR_RC, TIM17, GPIOB, GPIO_Pin_11);
