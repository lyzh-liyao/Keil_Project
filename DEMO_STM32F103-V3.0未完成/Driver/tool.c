...
void BaseClock_Init1(){
	...
	SysTick_Config(9000);//SYSTICKCOUNT 
...
/****************************************************
	...
	作者:	liyao 2015年9月8日14:10:51
****************************************************/
int8_t TimeTask_Add(int8_t title,SysTickStamp_t interval,void(*_task_handle)(void),TASK_MODE taskmode){
	uint8_t i = 0;  
...
//	TimeTask_t* lastTT = NULL;
	...
	while(curTT != NULL){
		...
		if(curTT->whileCount != 0){//当前任务在等待中
			...
			if(curTT->taskmode == Real_Mode)//实时模式
