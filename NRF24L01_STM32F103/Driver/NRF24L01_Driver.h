...
#define MAX_TX 		0x10 //status值 达到最大发送次数中断
#define TX_OK 		0x20 //status值 TX发送完成中断
#define RX_OK 		0x40 //status值 接收到数据中断
#define TX_FIFO		0x01 //status值 TX_FIFO满中断
//定义接收通道
#define Ch0	0x00
#define Ch1	0x02
#define Ch2	0x04
...
typedef enum {NO_MODE, RX_MODE, TX_MODE} WORK_MODE; 
...
struct _NRF24L01_T{
	...
	u8 (*nRF24L01_RxPacket)(NRF24L01_T* nrf);
	void (*Set_TX_Mode)(NRF24L01_T* nrf);
	void (*Set_RX_Mode)(NRF24L01_T* nrf);
	
	void (*NRF_IRQ_Handle)(NRF24L01_T* nrf);
	
	u8 TxBuf[TX_PLOAD_WIDTH];//{0x08,0xEF,0x03,0x04,0x05,0x06,0x07,0x08};
	u8 RxBuf[RX_PLOAD_WIDTH];//{0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01};
