  ...
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure);
 	...
 	//PWM模式设置 与通道设置TIM2_CH2
		...
		//TIM_OCInitStructure.TIM_Pulse = 0;					      				 //当计数器到达1000时,电平发送跳变
...
void TIM6_Config(void)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	/*基础设置72000000/预运行次数(1000)/满量程=预分频*/
	TIM_DeInit(TIM6);
	TIM_TimeBaseStructure.TIM_Period = 10;//计数值10000   
	TIM_TimeBaseStructure.TIM_Prescaler = 7200-1;    	//预分频,此值+1为分频的除数 65535以内
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;  	//采样分频
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up; 	//向上计数
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0; //起始值
...
void TIM3_Config(void)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	/*基础设置72000000/预运行次数(1000)/满量程=预分频*/
	TIM_DeInit(TIM3);
	TIM_TimeBaseStructure.TIM_Period = 36000;//计数值10000   
	TIM_TimeBaseStructure.TIM_Prescaler = 1000-1;    	//预分频,此值+1为分频的除数 65535以内
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;  	//采样分频
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up; 	//向上计数
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0; //起始值
