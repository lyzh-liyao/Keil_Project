...
static void _Put_Byte(MPU6050_T* mpu6050,uint8_t data){
	...
	*((&mpu6050->para1)+mpu6050->count) = data;
	mpu6050->count++;
	if(mpu6050->count == 9){
		mpu6050->X = ((short)((mpu6050->para2<<8) | mpu6050->para1))*180*10/32768 ;
		mpu6050->Y = ((short)((mpu6050->para4<<8) | mpu6050->para3))*180*10/32768 ;
		
		
		mpu6050->Z = ((short)((mpu6050->para6<<8) | mpu6050->para5))*1800/32768 ; 
		mpu6050->T = ((short)((mpu6050->para8<<8) | mpu6050->para7))/340.0+36.53 ; 
		if(mpu6050->T != 0 &&
			(abs(mpu6050->X - mpu6050->_Last_X) > (PWM_MAX/2) || 
			 abs(mpu6050->Y - mpu6050->_Last_Y) > (PWM_MAX/2) ||
			 abs(mpu6050->Z - mpu6050->_Last_Z) > (PWM_MAX/2))
		){
			mpu6050->MPU6050_Clean(mpu6050);
			mpu6050->X = mpu6050->_Last_X;
			mpu6050->Y = mpu6050->_Last_Y;
		}else{
...
MPU6050_T _Get_MPU6050(MPU6050_T* mpu6050){
	...
	for(i = 0; i < 11; i++){//每个包11个数据字节
		mpu6050->Put_Byte(mpu6050,_Usart3_Buff[22+i]);//第三个包为角度信息
	}
