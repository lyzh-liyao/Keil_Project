...
void RF_Write_Reg_send(u8 reg, u8 value)
{
    ...
    SPIx_Send(SPI1,reg);   //写寄存器地址
    SPIx_Send(SPI1,value); //写数据
...
u8 RF_Write_Buf_send(u8 reg, u8 *pBuf,u8 bytes)
{
    u8 status,i;
    CSN_L_2;
    status = SPIx_Send(SPI1,reg);//SPIx_ReadWriteByte(reg);    //选择要写入的寄存器
...
{  
    u8 reg_val;
    CSN_L_2;//CSN=0
    SPIx_Send(SPI1,reg);         //选择要读取的寄存器
    reg_val=SPIx_Send(SPI1,0xff);//读取数据
    CSN_H_2;//CSN=1
