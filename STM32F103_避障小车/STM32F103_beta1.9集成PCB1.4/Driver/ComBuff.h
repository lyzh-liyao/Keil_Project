...
#define SEND_BUFSIZE_USART 400	//串口发送缓冲区（发送协议和打印信息时的缓冲区）
#define SEND_BUFSIZE_BLUETOOTH 1000	//蓝牙发送缓冲区（发送协议和打印信息时的缓冲区）
#define SEND_BUFSIZE_NRF 1000	//无线模块发送缓冲区（发送协议和打印信息时的缓冲区）
#define RECV_BUFSIZE_UART4 33	//MPU6050接收缓冲区（发送协议和打印信息时的缓冲区）
extern QUEUE_T* Uart_Tx_Queue;
extern QUEUE_T* Nrf_Tx_Queue;
extern QUEUE_T* Bluetooth_Tx_Queue;
extern char _Uart4_Buff[RECV_BUFSIZE_UART4];
extern char BluetoothBuff[50]; 
