...
void Run_Protocol_Handle(PROTOCOL_INFO_T* pi){
	...
	#ifdef PRINT_ERR 
			printf("\r\n执行运动指令\r\n");
	    Send_To_Bluetooth(BluetoothBuff,sprintf(BluetoothBuff,"收到运动指令\r\n")); 
	#endif
	printf("\r\n执行运动指令:%d\r\n",pi->serial);
	//    Send_To_Bluetooth(BluetoothBuff,sprintf(BluetoothBuff,"收到运动指令\r\n")); 
//	#ifndef PRINT_ERR 
...
void State_Protocol_Send(){
	PROTOCOL_T pt = {0}; 
	pt.state_protocol.para1 = 1;//motor_L->DirCMD;//motor_L->Dir;
	pt.state_protocol.para2 = 0;
	pt.state_protocol.para3 = motor_L->SpeedCMD;//motor_L->Encoder_Diff;
	pt.state_protocol.para4 = 2;//motor_R->DirCMD;//motor_R->Dir;
	pt.state_protocol.para5 = 0;
	pt.state_protocol.para6 = motor_R->SpeedCMD;//motor_R->Encoder_Diff;
