...
void Motor_Init(void){
	...
	motor_L->Enc_Angle_Code = motor_R->Enc_Angle_Code = 1;				//度数对应编码值
}
...
static void _Motor_PID_Control(Motor_T* motor){
	...
	//------------------------PID计算-------------------------	
	if(motor->Cmd_Type == STRAIGHT){
		if(motor_L->DirCMD == motor_R->DirCMD)
	...
	}else if(motor->Cmd_Type == CUSTOM){
		if(motor->Encoder_CMD < INIT_ENC){
			if(motor->Encoder_CMD < motor->Encoder_Cur)
				PWM_Res = Motor_Speed_Calculate(motor, (motor->SpeedCMD * motor->Speed_Enc));
			else
				motor->Motor_Run(motor, MOTOR_STOP, 0);
		}else{
			if(motor->Encoder_CMD > motor->Encoder_Cur)
				PWM_Res = Motor_Speed_Calculate(motor, (motor->SpeedCMD * motor->Speed_Enc));
			else
