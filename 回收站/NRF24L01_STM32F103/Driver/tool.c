...
int32_t getMicrosecond(){
	...
	SysTick_Count = 1000000 - SysTick->VAL/9;
	if(SysTick_Count < 500000 && SysTick_Flag){
		SysTick_Flag = 0;
		second += 1000000;
	}else if(SysTick_Count > 500000 && SysTick_Flag == 0){
		SysTick_Flag = 1;
	}
	...
	//SysTick->CTRL |= CTRL_TICKINT_Set;//SysTick_ITConfig(ENABLE); 
...
void BaseClock_Init(){
	...
	//SysTick_SetReload(USMAX);   //9000/9Mhz
