...
u8  TX_ADDRESS[RX_ADR_WIDTH]= {0x10,0x19,0x90,0x04,0x19};    //发送到接收点的地址
...
//	return SPI_Send(SPI2,data); //返回接收的数据
//}
/**********************************************************
** 函数名: RF_Write_Reg
** 功能：写nRF24L01的寄存器
...
void RF_Write_Reg(u8 reg, u8 value)
{
    ...
    SPI_Send(SPI2,reg);   //写寄存器地址
    SPI_Send(SPI2,value); //写数据
...
{  
    u8 reg_val;
    CSN_L;//CSN=0
    SPI_Send(SPI2,reg);         //选择要读取的寄存器
    reg_val=SPI_Send(SPI2,0xff);//读取数据
    CSN_H;//CSN=1
...
u8 RF_Read_Buf(u8 reg, u8 *pBuf, u8 bytes)
{
     u8 status,i;
     CSN_L;//CSN=0
     status = SPI_Send(SPI2,reg);     //选择寄存器
    for(i=0;i<bytes;i++)
        pBuf[i] = SPI_Send(SPI2,0Xff);//连续读取bytes个字节数据
    CSN_H;////CSN=1
    return(status);
}
/**********************************************************
...
u8 RF_Write_Buf(u8 reg, u8 *pBuf,u8 bytes)
{
    u8 status,i;
    CSN_L;
    status = SPI_Send(SPI2,reg);//SPIx_ReadWriteByte(reg);    //选择要写入的寄存器
...
u8 nRF24L01_TxPacket(u8 *tx_buf)
{
	...
	{
		RF_Write_Reg(FLUSH_TX,0xf);//清除TX FIFO寄存器
		printf("最大重发！\r\n");
		return MAX_TX;
	}
