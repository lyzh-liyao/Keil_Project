...
static int8_t _recv_index, _is_FE = 0; 
...
void _clean_recv_buf(void){ 
	_is_FE = _recv_index = _recv_state = 0;
	memset(_recv_buf, 0, RECV_BUFSIZE); 
}
...
/****************************************************
	...
	作者:		liyao 2015年9月8日14:10:51
****************************************************/
int8_t Protocol_Put(uint16_t data){
	uint8_t i,loop, protocol_type = 0; 
	PROTOCOL_INFO_T* pi = NULL;  
	if(_recv_index == RECV_BUFSIZE)//避免数组越界
		_clean_recv_buf();  
	//--------存储数据----------
	...
	do{
		switch(_recv_state){
			...
			case 2: //校验和校验 
						...
						/*校验和暂时关闭*/
						if( 1==2 && (uint8_t)_sum != _recv_buf[_recv_index-2]){
							_clean_recv_buf();
						}else{
							_recv_state++;
							loop = 1;
